from fastapi import FastAPI, Security, HTTPException, Depends, status, Request
...
# --- Security ---
# Funktion zum Loggen hinzugefügt (nur für Debugging)
async def get_api_key(request: Request, api_key_header: str = Security(api_key_header)):
    """Validates the API Key provided in the header AND logs headers for debugging."""
    # Logge ALLE Header, die ankommen
    print(f"DEBUG: Received headers in get_api_key: {request.headers}")
    # Logge speziell den Wert des erwarteten API-Key-Headers
    print(f"DEBUG: Value received in '{API_KEY_NAME}' header: {api_key_header}")
    # Logge den Wert, der auf dem Server erwartet wird (zur Sicherheit kürzen!)
    print(f"DEBUG: Expected API_KEY (first 5 chars): {API_KEY[:5]}...")
